...
class PlayerSide:
    ...
    def __init__(self, player, HP):
        ...
        self.answer_queue = queue.Queue(maxsize=mudras_num_in_ninjutsu)


    def detect_mudra(self, webcam:int, player:str, target_queue:queue.Queue, answer_queue:queue.Queue):
    ...
    def judge_mudra(self, target_queue:queue.Queue, answer_queue:queue.Queue):
    ...
    def play(self, webcam:int, player:str, target_queue:queue.Queue, answer_queue:queue.Queue):
    ...
    def show_target_mudra(self, player:str, target_queue:queue.Queue):
        # 根據忍術顯示第(1~5)張
            # 傳輸WebCam影像給神經網路
            # 神經網路回傳判斷機率
    ...
    def start(self):
        player_t = threading.Thread(target=self.show_target_mudra, daemon=True, args=(self.player, self.target_queue))
        player_t.start()
